defmodule Quantity.MathTest do
  ...
  describe "add!" do
    test "raises ArgumentError when adding two quantities with different units" do
      ...
      assert_raise(
        ArgumentError,
        "arguments of different type: 1st argument #{inspect(a)} and 2nd argument #{inspect(b)}",
  ...
  describe "sum!/1" do
    test "raises ArgumentError when summing quantities with different units" do
      assert_raise(
        ArgumentError,
        "arguments of different type: ~Q[123 bananas], ~Q[456 apples]",
        fn -> Quantity.sum!([~Q[123 bananas], ~Q[456 apples]]) end
      )
    end

    test "raises ArgumentError when summing an empty list" do
      assert_raise(ArgumentError, "empty list", fn -> Quantity.sum!([]) end)
    end
  end
